...
import { usePhoneArchives } from "../PhoneArchivesProvider";
import { PhoneRecordCard } from "./PhoneRecordCard";
import type { FolderSelection } from "./FolderList";

type PhoneRecordListProps = {
  selectedFolder: FolderSelection;
};

export function PhoneRecordList({ selectedFolder }: PhoneRecordListProps) {
  const { records, folders, loading, loadByFolder } = usePhoneArchives();
    ...
    <div className="flex flex-col gap-4">
      ...
      {filtered.length === 0 ? (
        ...
        <ul className="space-y-3">
          {filtered.map((rec) => (
            <li key={rec.id}>
              <PhoneRecordCard record={rec} folders={folders} />
            </li>
          ))}
        </ul>
      )}
